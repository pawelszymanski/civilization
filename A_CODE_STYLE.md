...
### Formatter
Prettier is configured at `.prettierrc.json`. Run `npx prettier --write "src/**/*.ts"` to format. All TypeScript source files must be Prettier-clean before committing.
...
| `semi` | `true` | Semicolons always required |
| `bracketSpacing` | `true` | `{ x: 0 }` not `{x: 0}` — applies to both object literals and ES6 import braces |
| `printWidth` | `140` | Soft line-length limit |
| `trailingComma` | `"es5"` | Trailing commas in arrays and objects, not in function parameters |
| `arrowParens` | `"avoid"` | `s => s.x` not `(s) => s.x` for single-param arrow functions |
...
- Instantiated with `new class { ... }` pattern — tslint suppression on the line
- Instantiated in component via `new Worker(new URL('./path.worker', import.meta.url), { type: 'module' })`
