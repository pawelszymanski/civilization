...
export class TileUiService {
  ...
  public isTileOfScreenCoordsOnScreen(coordsOnCanvasPx: Coords): boolean {
    return (
      coordsOnCanvasPx.x + this.size.tile.width >= 0 &&
      coordsOnCanvasPx.x <= this.size.screen.width &&
      coordsOnCanvasPx.y + this.size.tile.height >= 0 &&
      coordsOnCanvasPx.y <= this.size.screen.height
    );
  }
